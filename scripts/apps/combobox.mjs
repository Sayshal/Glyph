...
/**
 * Render one `<li role="option">`.
 * @param {ComboboxOption} opt Option spec.
 * @param {string} id Combobox base id.
 * @param {number} gIdx Group index, `-1` for flat.
 * @param {number} oIdx Index within its group.
 * @param {string} selected Current selection, for pre-marking.
 * @returns {string} The option's HTML.
 */
function renderComboboxOption(opt, id, gIdx, oIdx, selected) {
  const isSelected = opt.value === selected;
  const keywords = [opt.label, ...(opt.tags ?? []), ...(opt.keywords ?? [])].join(' ');
  const icon = opt.icon ? `<img src="${esc(opt.icon)}" alt="">` : opt.iconClass ? `<i class="${esc(opt.iconClass)}" aria-hidden="true"></i>` : '';
  const tags = opt.tags?.length ? `<span class="glyph-combobox-option-tags">${opt.tags.map((t) => `<span class="glyph-combobox-option-tag">${esc(t)}</span>`).join('')}</span>` : '';
  const badge = opt.badge ? `<span class="glyph-combobox-option-badge">${esc(opt.badge)}</span>` : '';
  return `<li role="option" class="glyph-combobox-option${opt.disabled ? ' is-disabled' : ''}" id="${id}-opt-${gIdx}-${oIdx}"
    data-value="${esc(opt.value)}" data-label="${esc(opt.label)}" data-keywords="${esc(keywords)}"
    aria-selected="${isSelected}" ${isSelected ? 'data-selected' : ''} ${opt.disabled ? 'aria-disabled="true"' : ''}
    ${opt.description ? `data-tooltip="${esc(opt.description)}"` : ''}>
    <span class="glyph-combobox-option-icon">${icon}</span>
    <span class="glyph-combobox-option-label">${esc(opt.label)}</span>
    ${tags}
    ${badge}
  </li>`;
}
...
/**
 * Render a combobox shell: hidden input, trigger button, and searchable popover listbox.
 * @param {object} config
 * @param {string} config.id Base id for the trigger/listbox/options.
 * @param {string} config.name Form field name for the hidden input.
 * @param {string} [config.value] Currently-selected value.
 * @param {string} config.placeholder Trigger text shown with no selection.
 * @param {string} config.searchLabel Accessible label for the search input.
 * @param {string} config.noResultsText Text shown when a search matches nothing.
 * @param {ComboboxOption[]} [config.options] Flat options.
 * @param {ComboboxGroup[]} [config.groups] Grouped options - takes precedence over `options`.
 * @param {Record<string, string>} [config.data] Extra `data-*` attributes to set on the root.
 * @returns {string} The combobox's HTML.
 */
export function renderCombobox({ id, name, value = '', placeholder, searchLabel, noResultsText, options = [], groups = [], data = {} }) {
  ...
  const list = groups.length
    ? groups
        .map(
          (group, gIdx) => `<li class="glyph-combobox-group-label" role="presentation" id="${id}-grp-${gIdx}">${esc(group.label)}</li>
      <li role="group" aria-labelledby="${id}-grp-${gIdx}" class="glyph-combobox-group">
        <ul role="presentation" class="glyph-combobox-group-list">
          ${group.options.map((opt, oIdx) => renderComboboxOption(opt, id, gIdx, oIdx, value)).join('')}
        </ul>
      </li>`
        )
        .join('')
    : options.map((opt, oIdx) => renderComboboxOption(opt, id, -1, oIdx, value)).join('');
  return `<div class="glyph-combobox" data-combobox ${dataAttrs}>
    <input type="hidden" name="${esc(name)}" value="${esc(value)}">
    <button type="button" class="glyph-combobox-trigger" id="${id}-trigger" role="combobox" aria-haspopup="listbox" aria-expanded="false" aria-controls="${id}-listbox">
      <span class="glyph-combobox-trigger-icon" data-trigger-icon></span>
      <span class="glyph-combobox-trigger-label" data-trigger-label>${esc(placeholder)}</span>
      <span class="glyph-combobox-trigger-tags" data-trigger-tags></span>
      <i class="fa-solid fa-chevron-down glyph-combobox-caret" aria-hidden="true"></i>
    </button>
    <div class="glyph-combobox-panel" data-panel popover="manual" hidden>
      <div class="glyph-combobox-search">
        <i class="fa-solid fa-magnifying-glass" aria-hidden="true"></i>
        <input type="search" class="glyph-combobox-search-input" data-search autocomplete="off" spellcheck="false" aria-label="${esc(searchLabel)}">
      </div>
      <ul class="glyph-combobox-listbox" id="${id}-listbox" role="listbox" tabindex="-1">
        ${list}
        <li class="glyph-combobox-empty" role="presentation" hidden>${esc(noResultsText)}</li>
      </ul>
    </div>
  </div>`;
}
...
export class Combobox {
  ...
  /**
   * Attach to a single shell rendered by `renderCombobox`.
  ...
  constructor(root, opts) {
    ...
    this.empty = root.querySelector('.glyph-combobox-empty');
  ...
  #fitTags() {
    const options = this._open
      ? this.#options()
          .filter((o) => !o.hidden)
          .map((o) => ({ label: o.querySelector('.glyph-combobox-option-label'), tags: o.querySelector('.glyph-combobox-option-tags') }))
      : [];
  ...
  #triggerRow() {
    return { label: this.trigger.querySelector('.glyph-combobox-trigger-label'), tags: this.trigger.querySelector('.glyph-combobox-trigger-tags') };
  }
  ...
  #fitRows(rows) {
    ...
    for (const { tags } of valid) {
      ...
      for (const pill of tags.querySelectorAll('.glyph-combobox-option-tag')) pill.style.display = '';
    }
    const plans = valid.map(({ tags }) => {
      const pills = Array.from(tags.querySelectorAll('.glyph-combobox-option-tag'));
    ...
    for (const { tags, pills, avail, total, widths } of plans) {
      ...
      more.className = 'glyph-combobox-option-tag';
  ...
  #positionPanel() {
    ...
    const host = this.root.closest('.window-app, .application');
  ...
  #setActive(opt, { scroll = false } = {}) {
    for (const o of this.listbox.querySelectorAll('.glyph-combobox-option.is-active')) o.classList.remove('is-active');
    if (!opt) {
      this._activeId = null;
  ...
  #renderTrigger(opt) {
    ...
    const srcIcon = opt.querySelector('.glyph-combobox-option-icon')?.firstElementChild;
    this.triggerIcon.replaceChildren(...(srcIcon ? [srcIcon.cloneNode(true)] : []));
    const optTags = opt.querySelector('.glyph-combobox-option-tags');
    this.triggerTags.replaceChildren(...(optTags ? [optTags.cloneNode(true)] : []));
  ...
  #applyFilter(query) {
    ...
    this.listbox.querySelectorAll('.glyph-combobox-group-label').forEach((label) => {
